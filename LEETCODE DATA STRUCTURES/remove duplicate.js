...
var removeDuplicates = function(nums) {
    ...
    let j = 0; // pointer to track the position of the last unique element
    
    for (let i = 1; i < nums.length; i++) {
        if (nums[i] !== nums[j]) {
            j++; // move the pointer forward
            nums[j] = nums[i]; // place the current unique element in the correct position
        }
    }
    
    return j + 1; // the count of unique elements
};
